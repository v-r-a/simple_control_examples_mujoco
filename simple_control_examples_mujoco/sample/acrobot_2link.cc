// Copyright 2024 Vyankatesh Ashtekar
//
...
double my_mju_powerbalance(const mjModel *mm, mjData *dd)
{
    ...
    dd->userdata[11] = dd->energy[0] + dd->energy[1];                           // Energy now: TE = PE + KE
    double de = (dd->userdata[11] - dd->userdata[10]) / (1 * mm->opt.timestep); // Energy dissipation or addition rate
    dd->userdata[10] = dd->userdata[11];                                        // Update the old value of energy
    printf("de: %lg act_power: %lg\n", de, actuator_power);
    double residual = (de - actuator_power) / (1); // Error (Watt)/( Watt Normalizer)
    return residual;
}
...
{
    // Angular momentum based controller

    mjtNum L = -1;   // angular momentum about base point
    mjtNum dL = -1;  //
    mjtNum ddL = -1; //
    mjtNum td = 0;   // holding torque
    int csi = 0, vcsi = 1, lsi = 2;

    mjtNum pp = 1, Kp = 4 * mju_pow(pp, 3), Kd = 6 * mju_pow(pp, 2) + 1 * mju_pow(pp, 0.25), Kdd = 4 * pp + 0.1;
    // mjtNum Kp = 02, Kd = 0.5 , Kdd = 0.1;
    mjtNum temp[3], temp2[3];
    mjtNum cgpos[3] = {d->sensordata[csi], d->sensordata[csi + 1], d->sensordata[csi + 2]};

    // Compute linear momentum of the CG = Mtot*CGvel
    mjtNum linmomcg[3] = {d->sensordata[3 * vcsi], d->sensordata[3 * vcsi + 1], d->sensordata[3 * vcsi + 2]};
    mjtNum totalmass = m->body_mass[1] + m->body_mass[2];
    mju_scl3(linmomcg, linmomcg, totalmass);
    // Rcg x Mtot*CGvel
    mju_cross(temp, cgpos, linmomcg);
    // mju_printMat(temp,3,1);
    // Angular momentum about base point = angular momentum abt CG + Rcg x Mtot*CGvel
    L = d->sensordata[3 * lsi] + temp[0];

    // Compute L dot = gravitation torque about the base point Rcg x Mtot*Gravity
    mjtNum mgforce[3] = {0, 0, -9.81};
    mju_scl3(mgforce, mgforce, totalmass);
    mju_cross(temp2, cgpos, mgforce);
    dL = temp2[0]; //-(d->sensordata[csi + 1])*(totalmass)*9.81;
    // mju_printMat(temp2,3,1);
    // Compute L ddot = -(m1+m2)g dXg
    ddL = -(d->sensordata[3 * vcsi + 1]) * (totalmass)*9.81;

    // ctrl[0] because only one motor is defined at joint 2.
    (d->ctrl[0]) = Kdd * ddL + Kd * dL + Kp * L + 0;
    printf("Kp: %lg\tKd: %lg\tKdd: %lg\n", Kp, Kd, Kdd);
    // printf("ddL: %lg and dXg: %lg\n", ddL, d->sensordata[3*vcsi + 1]);

    // Trials
    // id 0
    // int csi = mj_name2id(m,mjOBJ_SENSOR,"COM");
    // id 1
    // int vcsi = mj_name2id(m,mjOBJ_SENSOR,"comvel");
    // id 2
    // int lsi = mj_name2id(m,mjOBJ_SENSOR,"AngMom");

    // printf("COM: %lg %lg %lg\n", d->sensordata[csi], d->sensordata[csi+1], d->sensordata[csi+2]);
    // printf("COM vel: %lg %lg %lg\n", d->sensordata[3*vcsi], d->sensordata[3*vcsi+1], d->sensordata[3*vcsi+2]);
    // printf("ANG MOM: %lg %lg %lg\n\n", d->sensordata[3*lsi], d->sensordata[3*lsi+1], d->sensordata[3*lsi+2]);

    double pb = my_mju_powerbalance(m, d);
    printf("power balance residual: %lg\n", pb);
}
