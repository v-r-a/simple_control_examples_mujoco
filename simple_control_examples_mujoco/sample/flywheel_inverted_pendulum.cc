...
{
    double Lx=-1; // angular momentum about CG in x direction
    double Kp = 1300, Kd = 30, Ki = 0., err, derr;
    
    err = (0 - (d->qpos[0]));  // Set point regulation theta eq = 0
    derr = (0 - (d->qvel[0])); // Set point regulation theta dot eq = 0
    d->userdata[0] = 0;
    d->userdata[0] += err;
    // ctrl[0] becuase only one actuator is defined
    d->ctrl[0] = -Kp * err - Kd * derr - Ki * (d->userdata[0]);
