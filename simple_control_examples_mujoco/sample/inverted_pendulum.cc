...
{
    double Kp = 40, Kd = 1, Ki = 0.1, err, derr;
    err = (0 - (d->qpos[1]));  // Set point regulation theta eq = 0
    derr = (0 - (d->qvel[1])); // Set point regulation theta dot eq = 0
    d->userdata[0] = 0;
    d->userdata[0] += err;
    d->ctrl[0] = -Kp * err - Kd * derr - Ki * (d->userdata[0]);
    // if (m->nu == m->nv)
    //  mju_scl(d->ctrl, d->qvel, -10, m->nv);
}
